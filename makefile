...
payload_key_id_output:
	aws --region $(REGION) \
		cloudformation describe-stacks \
		--query 'Stacks[?StackName==`$(DEPLOYED_INTERNAL_STACK)`].Outputs[0][?OutputKey==`KeyPairId`].OutputValue' \
...
payload_public_dns_output:
	aws --region $(REGION)\
		cloudformation describe-stacks \
		--query 'Stacks[?StackName==`$(DEPLOYED_INTERNAL_STACK)`].Outputs[1][?OutputKey==`InstanceDnsName`].OutputValue' \
